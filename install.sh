...
# Usage: curl -sSL https://raw.githubusercontent.com/ahonnecke/codex-cli-compact/main/install.sh | bash
...
BASE_URL="https://raw.githubusercontent.com/ahonnecke/codex-cli-compact/main"

echo "[install] Downloading core engine..."
curl -fsSL "$BASE_URL/bin/dual_graph_launch.sh" -o "$INSTALL_DIR/dual_graph_launch.sh" && chmod +x "$INSTALL_DIR/dual_graph_launch.sh"
curl -sf  "$BASE_URL/bin/version.txt" -o "$INSTALL_DIR/version.txt" 2>/dev/null || true
