...
curl -sSL "$BASE_URL/dual_graph_launch.sh" -o "$INSTALL_DIR/dual_graph_launch.sh" && chmod +x "$INSTALL_DIR/dual_graph_launch.sh"
...
curl -sSL "$BASE_URL/dg-bench"  -o "$INSTALL_DIR/dg-bench"  && chmod +x "$INSTALL_DIR/dg-bench"
...
echo "  dgc /path/to/project   # Claude Code (local MCP auto-setup)"
echo "  dg  /path/to/project   # Codex CLI   (local MCP auto-setup)"
